              ...
              <div className="flex-1 min-w-0 flex flex-col gap-1.5">
                <div className="min-w-0 flex items-start justify-between gap-2">
                  <div className="min-w-0 flex-1">
                    ...
                    {/* mydig link sits with the identity (right under the
                        name), not down in the external-link group — it's
                        the native in-app destination, @instagram / Discogs
                        are outward links. Skipped until a username is
                        claimed. */}
                    {data.mydig && (
                      <Link
                        to={`/my/${data.mydig.username}`}
                        className="mt-1 inline-flex items-center gap-1.5 truncate text-accent hover:text-accent-hover"
