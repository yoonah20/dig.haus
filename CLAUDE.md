...
- `jobs/` — `rankScheduler` (00:00 KST), `labelFeedPoller` (03:00), `usageLogPruner` (04:00, 90-day retention), `releaseSyncJob` (04:00 — re-resolves store links for week-old releases + enqueues the release-day review crawl once; no boot catch-up, so a missed tick is recovered via the batch admin trigger `POST /api/admin/run-release-sync` — "릴리스 싱크 수동 실행" button in `/admin/api`), `coverCachePruner` (04:30 — LRU eviction of `cover-cache/` above 2 GiB)
