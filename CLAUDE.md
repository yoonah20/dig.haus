...
| `/admin` (+ `/admin/curation`, `/admin/api`, `/admin/maintenance`, `/admin/tags`) | `pages/Admin.tsx` | One component, five tabs; maintenance tab hosts the 정리 (duplicate-album merge) tools; 태그 tab (`pages/AdminTags.tsx`, lazy) is the catalog-wide tag cleanup workspace — a 태그 목록/태그 없는 앨범 view toggle (merge/rename + blacklist with bulk/drag/select-all + junk filter on the list; a re-tagging worklist of albums whose display tags are empty on the other) plus the `TagBlacklistPanel` viewer (moved here from the 큐레이션 tab so all tag work is in one place) |
...
- **바구니 (default cart) — the `is_default` flag is live again (2026-08-01).** The old 굿굿/별루 defaults were dropped 2026-05-17, leaving `is_default` dormant; it now marks the per-user **바구니**, a private (`is_public=0`) undeletable staging crate that behaves like a shopping cart. Exactly one per user, seeded by `seed-default-cart-2026-08-01` and lazily find-or-created in `routes/crates.ts` (`ensureDefaultCart`). Cart endpoints under `/api/mydig/cart` (GET contents / POST item / DELETE item / DELETE all) key off the caller's `is_default` box and resolve albums by slug/mbid (`resolveAlbumPk`) so the client never handles the crate id. Because it's private, a cart add never inflates album `crate_count` (public-only) and it doesn't surface on the mydig floor — but it DOES appear in the album-page 담기 dropdown (owner sees all own crates) and counts toward 담기 membership. UI: one-click `CartChip` beside the ▶ on browse-grid album cards (`showCartChip` prop, home feed + 최근 발매 + `/dig`, suppressed at compact density) feeds `FloatingCart` — a bottom-right basket widget mounted once at App root (`useCart` hook, optimistic add/remove/clear). The rename/delete lock in `routes/crates.ts` (already keyed on `is_default`) protects it for free.
...
**LLM routing** — every default path runs on **DeepSeek v4 Flash** (model id `deepseek-v4-flash`): scrape extraction, manual review, editorial pick, pronunciation, similar-album descriptions, Korean review summary. **Anthropic Haiku was dropped from every default path on 2026-07-06** (constants and pricing rows remain only for env-override use). **The `deepseek-chat` alias was retired by DeepSeek 2026-07-25** — the API now 400s on it (`supported names are deepseek-v4-pro / deepseek-v4-flash`), so `DEEPSEEK_MODEL` and every call-site default are the explicit `deepseek-v4-flash` id. The blanket primary model is **admin-editable at runtime** via the `app_settings.llm_primary_model` key (dropdown of suggested ids **plus a free-text field** in `/admin/api`, `GET/POST /api/admin/llm-model`, `utils/settings.ts`) — switch flash ↔ pro or type a future/other model id without a redeploy. The POST validates a loose id shape only (not a hard allowlist). **llmAdapter routing** (`callLlmByModel`): `compat:<model>` → generic OpenAI-compatible provider (`services/openaiCompat.ts`), `deepseek-*` → DeepSeek, any other id → Anthropic. The compat provider covers OpenAI/Groq/OpenRouter/Together/local etc. via one `/chat/completions` branch — base URL from `LLM_COMPAT_BASE_URL` env or `app_settings.llm_compat_base_url` (admin `/admin/api`, `GET/POST /api/admin/llm-compat`), API key from `LLM_COMPAT_API_KEY` env only (never stored/shown). The review scrape/manual path (`extractJsonWithFallback` in `services/reviews.ts`) now also routes through `callLlmByModel`, so a `compat:`/Anthropic primary works for extraction too, not just the invokeLlm ops. Pricing rows exist only for DeepSeek/Anthropic ids, so compat usage-log rows fall back to the default (Haiku) rate on the admin cost panel — approximate, not exact. Precedence in `resolvePrimaryModel`: per-op env > blanket env > `app_settings` value > code default. Routing machinery:
...
- `jobs/` — `rankScheduler` (00:00 KST), `labelFeedPoller` (03:00), `usageLogPruner` (04:00, 90-day retention), `releaseSyncJob` (04:00 — re-resolves store links for week-old releases + enqueues the release-day review crawl once; in-process cron was silently missing days when the host restarted through 04:00, so a **boot catch-up** now runs it once per KST day if the tick was missed, guarded by the `app_settings.release_sync_last_run` date. Also swept via the batch admin trigger `POST /api/admin/run-release-sync` — "릴리스 싱크 수동 실행" button in `/admin/api`, which shows last-run vs today KST), `coverCachePruner` (04:30 — LRU eviction of `cover-cache/` above 2 GiB)
