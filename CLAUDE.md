...
Current per-album cost is **~$0.01** for a typical 9-15 review pull (per-review unit cost ~$0.001), via the `scrapeReviewFromUrl` path. Page fetches go through Jina Reader (`r.jina.ai/`) — free proxy that renders JS and converts to clean markdown, so the LLM sees the article instead of HTML boilerplate. URL discovery happens via one of three selectable engines (admin clicks 🔎 자동 검색 → engine select next to the button): **Serper** (Google SERP proxy, $5/2.5k credits one-time free), **Tavily** (AI-search aggregator, 1000/mo free no card), **Brave Search** ($5/1k with $5/mo credit ≈ 1000/mo free, no card). Returns 10–20 candidates, the editorial picker LLM picks the actually-editorial ones (~$0.0003 per pick call). The selector lets the operator A/B engines on the same album — useful since each backend's SERP shape differs (Serper especially showed gl: us bias even with explicit gl: kr params, so KR-ranked review sites went missing on some albums). `services/googleCse.ts` is a dormant fourth option behind `GOOGLE_CSE_API_KEY` + `GOOGLE_CSE_ID`, blocked on Google deprecating "Search the entire web" for new Programmable Search Engines.
...
- `server/src/services/` — `claude` (Anthropic client + pronunciation + summary + similar-descriptions), `reviews` (3-step review pipeline), `albumUrlExtract`, `musicbrainz`, `lastfm`, `discogs`, `spotify`, `youtube`, `bandcamp`, `serper` (active discovery), `braveSearch` / `googleCse` (dormant alts), `jina`, `deepseek`, `exchangeRates`, `avatarHost`, `customCoverHost`
