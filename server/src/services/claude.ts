...
async function _generateSimilarDescriptions(
  ...
  try {
    const promptText = `"${baseAlbum}" (${baseArtist})와 아래 앨범들이 어떻게 닮았는지 각 **2-3문장** 한국어로 설명해주세요.

**규칙**:
- 설명 안에서 앨범 제목 "${baseAlbum}" **또는** 아티스트명 "${baseArtist}" 중 **하나만 골라** 호명하며 비교할 것 (둘 다 넣지 말 것 — 문장이 길고 어색해짐). 예1: "${baseAlbum}의 어두운 드론 질감이 이 앨범에서도…". 예2: "${baseArtist}의 보컬 접근법과 유사하게…".
- '기준 앨범', '이 앨범', '해당 작품', '앞서 소개한 앨범' 같은 지시어 **금지** — 어색하게 읽힘.
- 사운드·분위기·장르·구조·보컬·프로덕션 중 **구체적으로 어떤 면**이 공통되는지 명시. 한 문장은 공통점, 다른 문장은 이 앨범 고유의 특색을 덧붙이는 식으로 풀어도 좋음.
- 개별 앨범을 단순 기술 금지. 항상 연결점 중심 서술.
