...
export async function generateKoreanSummary(
  ...
  try {
    ...
    const promptText =
      ...
      `**정확히 3문장 / 총 길이 210-270자**. 점수 옆 float 박스는 약 5줄 wrap을 기준으로 디자인되어 있고, 그보다 길어지는 줄은 박스 아래로 자연스럽게 흘러내리도록 되어 있다(정상 동작). 너무 짧으면(200자 미만) 점수 옆이 비어 보이고 내용이 빈약하며, 420자를 넘기면 카드가 과하게 부풀어 보임. 문장당 70-90자 정도가 자연스럽다. ` +
